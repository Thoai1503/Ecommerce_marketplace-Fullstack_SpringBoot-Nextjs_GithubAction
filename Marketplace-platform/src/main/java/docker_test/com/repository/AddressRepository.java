...
public class AddressRepository implements IRepositories<Address> {
	...
	public Address Create(Address item) throws SQLException {
		...
		try (Connection conn = dbConnection.getConn();
			...
			if (shopId != null && shopId > 0) {
				stmt.setObject(2, shopId, Types.BIGINT);
			} else {
	...
	public Address getByShopId(long shopId) {
		...
		try (Connection conn = dbConnection.getConn(); PreparedStatement stmt = conn.prepareStatement(sql)) {
			...
			if (rs.next()) {
				return new Address(rs.getLong("id"), rs.getLong("user_id"), rs.getLong("shop_id"),
						rs.getString("recipient_name"), rs.getString("recipient_phone"), rs.getString("address_line"),
						rs.getLong("ward"), rs.getLong("district"), rs.getLong("city"), rs.getString("postal_code"),
						rs.getInt("is_default"), rs.getTimestamp("created_at").toLocalDateTime(),
	...
	public List<Address> findByUserId(long userId) {
		...
		try (Connection conn = dbConnection.getConn(); PreparedStatement stmt = conn.prepareStatement(sql)) {
			...
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
				Address address = new Address(rs.getLong("id"), rs.getLong("user_id"), rs.getLong("shop_id"),
						rs.getString("recipient_name"), rs.getString("recipient_phone"), rs.getString("address_line"),
						rs.getLong("ward"), rs.getLong("district"), rs.getLong("city"), rs.getString("postal_code"),
						rs.getInt("is_default"), rs.getTimestamp("created_at").toLocalDateTime(),
						rs.getTimestamp("updated_at").toLocalDateTime());
