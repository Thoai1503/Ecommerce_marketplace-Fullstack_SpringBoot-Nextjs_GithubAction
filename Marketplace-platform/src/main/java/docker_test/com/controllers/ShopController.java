...
public class ShopController {
	...
	public ResponseEntity<?> checkShop(@RequestParam("user_id") int userId) {
		...
		if (hasShop) {
			...
			isComplete = businessLicense != null && !businessLicense.isBlank() && taxCode != null && !taxCode.isBlank();
		}
	...
	@PutMapping("/{id}")
	public ResponseEntity<?> updateCompliance(@PathVariable int id, @RequestBody Map<String, String> payload) {
		...
		String ownerName = payload.getOrDefault("owner_name", existing.getOwner_name());
		String urlCardFront = payload.getOrDefault("url_card_front", existing.getUrl_card_front());
		String urlCardBack = payload.getOrDefault("url_card_back", existing.getUrl_card_back());
