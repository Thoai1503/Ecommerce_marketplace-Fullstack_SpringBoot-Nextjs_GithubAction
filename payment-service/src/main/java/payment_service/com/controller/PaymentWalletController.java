...
public class PaymentWalletController {
	...
	private PaymentWalletResponseDTO toWalletResponse(PaymentWallet wallet) {
		PaymentWalletStatus status = wallet.getStatus() == null ? PaymentWalletStatus.ACTIVE : wallet.getStatus();

		return PaymentWalletResponseDTO.builder().id(wallet.getId()).userId(wallet.getUserId())
				.balance(wallet.getBalance()).lockedBalance(wallet.getLockedBalance()).currency(wallet.getCurrency())
				.isActive(status == PaymentWalletStatus.ACTIVE).status(status.name()).createdAt(wallet.getCreatedAt())
				.updatedAt(wallet.getUpdatedAt()).build();
	}

	@PostMapping("/{userId}/credit")
	public ResponseEntity<Map<String, String>> creditWallet(@PathVariable Long userId, @RequestParam Long amount,
			@RequestParam(required = false) String refType, @RequestParam(required = false) Long refId,
	...
	public ResponseEntity<Map<String, String>> debitWallet(@PathVariable Long userId, @RequestParam Long amount,
			@RequestParam(required = false) String refType, @RequestParam(required = false) Long refId,
			@RequestParam(required = false) String description) {
