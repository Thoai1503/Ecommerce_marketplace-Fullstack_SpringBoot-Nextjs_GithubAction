...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 TRIGGER `trg_adj_item_before_insert` BEFORE INSERT ON `shipment_adjustment_item` FOR EACH ROW BEGIN
  ...
  IF v_req_status IS NULL THEN
    ...
    SET MESSAGE_TEXT = 'Invalid adjustment_request_id';
  END IF;

  IF v_req_status <> 'PENDING_BUYER' THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Cannot add items unless request is PENDING_BUYER';
  END IF;
  ...
  IF v_unit_price IS NULL THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'order_item not found or does not belong to order';
  END IF;

  IF NEW.new_quantity IS NULL OR NEW.new_quantity < 0 OR NEW.new_quantity > v_old_qty THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'new_quantity must be between 0 and old_quantity';
  END IF;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 TRIGGER `trg_adj_item_after_insert` AFTER INSERT ON `shipment_adjustment_item` FOR EACH ROW BEGIN
  CALL sp_adj_recalc_request_totals(NEW.adjustment_request_id);
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 TRIGGER `trg_adj_item_before_update` BEFORE UPDATE ON `shipment_adjustment_item` FOR EACH ROW BEGIN
  DECLARE v_req_status VARCHAR(50);
  ...
  IF v_req_status <> 'PENDING_BUYER' THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Cannot edit items unless request is PENDING_BUYER';
  END IF;

  IF NEW.new_quantity < 0 OR NEW.new_quantity > OLD.old_quantity THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'new_quantity must be between 0 and old_quantity';
  END IF;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 TRIGGER `trg_adj_item_after_update` AFTER UPDATE ON `shipment_adjustment_item` FOR EACH ROW BEGIN
  CALL sp_adj_recalc_request_totals(NEW.adjustment_request_id);
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 TRIGGER `trg_adj_item_before_delete` BEFORE DELETE ON `shipment_adjustment_item` FOR EACH ROW BEGIN
  DECLARE v_req_status VARCHAR(50);
  ...
  IF v_req_status <> 'PENDING_BUYER' THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Cannot delete items unless request is PENDING_BUYER';
  END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 TRIGGER `trg_adj_item_after_delete` AFTER DELETE ON `shipment_adjustment_item` FOR EACH ROW BEGIN
  CALL sp_adj_recalc_request_totals(OLD.adjustment_request_id);
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 TRIGGER `trg_adj_req_before_insert` BEFORE INSERT ON `shipment_adjustment_request` FOR EACH ROW BEGIN
  DECLARE v_pending_count INT DEFAULT 0;

  IF NEW.order_shipment_id IS NULL OR NEW.order_id IS NULL OR NEW.shop_id IS NULL THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'order_shipment_id, order_id, shop_id are required';
  END IF;
  ...
  IF NEW.status <> 'PENDING_BUYER' THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'New adjustment request must start with PENDING_BUYER';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'order_shipment does not match order_id/shop_id';
  END IF;
  ...
  IF v_pending_count > 0 THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Only one PENDING_BUYER request is allowed per shipment';
  END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 TRIGGER `trg_adj_req_after_insert` AFTER INSERT ON `shipment_adjustment_request` FOR EACH ROW BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 TRIGGER `trg_adj_req_before_update` BEFORE UPDATE ON `shipment_adjustment_request` FOR EACH ROW BEGIN
  IF OLD.status IN ('ACCEPTED_BY_BUYER','REJECTED_BY_BUYER','CANCELLED_BY_SHOP','EXPIRED')
     AND NEW.status <> OLD.status THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Cannot transition from a terminal adjustment status';
  END IF;
  ...
  IF OLD.status = 'PENDING_BUYER' THEN
    IF NEW.status NOT IN ('PENDING_BUYER','ACCEPTED_BY_BUYER','REJECTED_BY_BUYER','CANCELLED_BY_SHOP','EXPIRED') THEN
      SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Invalid status transition from PENDING_BUYER';
    END IF;

    IF NEW.status = 'ACCEPTED_BY_BUYER' THEN
      IF (SELECT COUNT(*) FROM shipment_adjustment_item i WHERE i.adjustment_request_id = OLD.id) = 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Cannot accept adjustment request without any items';
      END IF;
    END IF;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 TRIGGER `trg_adj_req_after_update` AFTER UPDATE ON `shipment_adjustment_request` FOR EACH ROW BEGIN
  ...
  IF OLD.status = 'PENDING_BUYER' AND NEW.status = 'ACCEPTED_BY_BUYER' THEN
    ...
    IF NOT EXISTS (
      SELECT 1
      FROM shipment_adjustment_financial f
...
/*!50106 DROP EVENT IF EXISTS `ev_expire_adjustment_requests` */;
DELIMITER ;;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;;
...
/*!50106 CREATE*/ /*!50117 DEFINER=`root`@`%`*/ /*!50106 EVENT `ev_expire_adjustment_requests` ON SCHEDULE EVERY 5 MINUTE STARTS '2026-04-16 03:25:35' ON COMPLETION NOT PRESERVE ENABLE DO UPDATE shipment_adjustment_request
  SET status = 'EXPIRED',
...
--
/*!50003 DROP PROCEDURE IF EXISTS `sp_adj_recalc_request_totals` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `sp_adj_recalc_request_totals`(IN p_request_id BIGINT)
BEGIN
  UPDATE shipment_adjustment_request r
  SET r.total_original_amount = (
        SELECT IFNULL(SUM(i.old_total),0)
        FROM shipment_adjustment_item i
        WHERE i.adjustment_request_id = r.id
      ),
      r.total_adjusted_amount = (
        SELECT IFNULL(SUM(i.new_total),0)
        FROM shipment_adjustment_item i
        WHERE i.adjustment_request_id = r.id
      ),
      r.total_diff_amount = (
        SELECT IFNULL(SUM(i.diff_total),0)
        FROM shipment_adjustment_item i
        WHERE i.adjustment_request_id = r.id
      )
...
-- Dump completed on 2026-04-16 10:26:32
