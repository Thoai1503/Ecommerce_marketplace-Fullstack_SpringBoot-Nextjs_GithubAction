...
import { 
  ChevronLeft, Plus, Edit3, Trash2, Settings, List, ArrowLeft, Layers, Scale, Sparkles
} from 'lucide-react';
...
export default function AttributeDetail() {
  ...
  const { attribute, values, isLoading, createValue, updateValue, deleteValue, isUpdatingValues } = useAttributeDetail(id || '');
  ...
  if (!attribute) return (
    <div className="p-20 text-center flex flex-col items-center">
      ...
      <h3 className="text-xl font-bold text-slate-800">Attribute not found</h3>
      <button onClick={() => router.push('/admin/categories/attributes')} className="mt-4 text-blue-600 hover:underline">Back to list</button>
    </div>
  );
  ...
  const handleDeleteValue = async (valueId: string) => {
    if (confirm("Are you sure you want to delete this value?")) {
      try {
        ...
        setToast({ id: Date.now().toString(), message: "Value deleted.", type: "success" });
      } catch(e) {
  ...
  const handleSaveModalValues = async (valuesArray: string[]) => {
    try {
      if (editingValue && valuesArray.length > 0) {
        // Update the editing value with the first value from the array
        await updateValue({ 
        ...
        setToast({ id: Date.now().toString(), message: "Value updated.", type: "success" });
        setIsModalOpen(false);
        setEditingValue(null);
      } else if (valuesArray.length > 0) {
        // Create new values
        const maxOrder = values.length > 0 ? Math.max(...values.map(v => v.displayOrder)) : 0;
        ...
        setToast({ id: Date.now().toString(), message: `Added ${valuesArray.length} value(s) successfully.`, type: "success" });
        setIsModalOpen(false);
      }
    } catch(e) {
      setToast({ id: Date.now().toString(), message: "Operation failed.", type: "error" });
    }
  };
  ...
  const handleQuickAdd = async (e?: React.FormEvent) => {
    ...
    try {
      const newValues = quickValue.split(',').map(v => v.trim()).filter(v => v);
      ...
      setQuickValue('');
      setToast({ id: Date.now().toString(), message: `Added ${newValues.length} value(s) successfully.`, type: "success" });
    } catch (err) {
      setToast({ id: Date.now().toString(), message: "Failed to add values.", type: "error" });
    } finally {
    ...
    <div className="p-6 lg:p-10 animate-in fade-in duration-500 max-w-5xl mx-auto pb-24 space-y-8">
      ...
      <AttributeValueModal 
        ...
        attribute={attribute ? {
          id: attribute.id,
          name: attribute.name,
          slug: attribute.attributeCode || attribute.id,
          data_type: attribute.option === 'DROPDOWN' ? 1 : 2
        } as any : null}
        initialValues={editingValue ? [editingValue.value] : []}
      />
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 text-sm text-slate-500 font-medium mb-1">
              <span className="hover:text-blue-600 cursor-pointer" onClick={() => router.push('/admin/categories/attributes')}>Attribute</span>
              <span>/</span>
              <span>{attribute.name}</span>
            </div>
            <div className="flex items-center gap-3">
               <h1 className="text-2xl font-black text-slate-800 tracking-tight">Attribute: {attribute.name}</h1>
               {attribute.published ? (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wider bg-green-50 text-green-700 border border-green-200">Active</span>
               ) : (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wider bg-slate-100 text-slate-500 border border-slate-200">Hidden</span>
               )}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button 
            onClick={() => router.push(`/admin/categories/attributes/${attribute.id}/edit`)}
         ...
         <div className="space-y-6">
            <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm p-6">
               ...
               <div className="space-y-4">
                  <div>
                     <p className="text-[10px] font-bold text-slate-400 uppercase">Code ID</p>
                     <p className="text-lg font-mono font-black text-slate-800">{attribute.attributeCode}</p>
                  </div>
                  <div>
                     <p className="text-[10px] font-bold text-slate-400 uppercase">Option Type</p>
                     <p className="text-sm font-bold text-slate-700 bg-slate-50 inline-block px-2 py-1 rounded border border-slate-100 mt-1">
                        {attribute.option === 'DROPDOWN' ? 'Dropdown Select' : 'Radio Button'}
                     </p>
                  </div>
                     ...
                     {linkedUnit ? (
                       <div className="flex items-center gap-2 mt-1 p-2 bg-blue-50 border border-blue-100 rounded-lg">
                          ...
                          <p className="text-sm font-bold text-blue-700">
                             {linkedUnit.label} <span className="text-blue-400 font-mono">({linkedUnit.symbol})</span>
                          </p>
                       </div>
                     ) : (
                       <div className="flex items-center gap-2 mt-1">
                          <p className="text-sm font-bold text-slate-400">None (No unit linked)</p>
                       </div>
                     )}
                  </div>

                  <div>
                     <p className="text-[10px] font-bold text-slate-400 uppercase">Values Count</p>
                     <p className="text-lg font-black text-blue-600">{values.length}</p>
                  </div>
               </div>
            </div>
            ...
            <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm p-6">
               ...
               {isLoadingCategories ? (
                  ...
                  <div className="space-y-2">
                     {linkedCategories.map(cat => (
                        <div 
                           ...
                           onClick={() => router.push(`/admin/categories/industries/${cat.id}`)}
                        >
                  ...
                  <div className="p-4 bg-slate-50 rounded-xl text-center">
                     <p className="text-xs text-slate-400">Not assigned to any category yet.</p>
                     <button 
            ...
            <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
                  ...
                  <form onSubmit={handleQuickAdd} className="flex gap-2">
                     <div className="relative flex-1">
                        <input 
                           ...
                           placeholder={linkedUnit ? `Enter value (e.g. 10, 20)...` : `Enter value (e.g. Red, Blue, Green)...`}
                        />
                        {linkedUnit && (
                     ...
                     >
                        {isQuickAdding ? 'Adding...' : <><Plus size={16} /> Quick Add</>}
                     </button>
                  </form>
               </div>
               ...
               {values.length === 0 ? (
                  <div className="flex-1 flex flex-col items-center justify-center p-10 text-slate-400">
                     ...
                     <p className="text-xs mt-1">Use the Quick Add above to start.</p>
                  </div>
               ) : (
                  <div className="overflow-x-auto">
                     <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-50">
                           {values.map((val) => (
                              <tr key={val.id} className="hover:bg-slate-50/80 transition-colors group">
                                 ...
                                 <td className="px-6 py-4">
                                    <div className="flex items-center gap-2">
                                       ...
                                       <span className="text-sm font-bold text-slate-800">
                                          {val.value} {linkedUnit ? <span className="text-slate-400 font-normal">({linkedUnit.symbol})</span> : ''}
                                       </span>
                                    </div>
                                 </td>
                                 <td className="px-6 py-4 text-right">
                                    <span className="text-xs font-medium text-slate-500">{new Date(val.createdAt).toLocaleDateString()}</span>
                                 </td>
                                 <td className="px-6 py-4 text-right">
