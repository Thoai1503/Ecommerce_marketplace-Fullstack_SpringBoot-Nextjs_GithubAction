...
import { 
  ...
  Ticket, Wallet, Layers, ShieldCheck, Settings,
  ChevronDown, LogOut, ChevronLeft, X
} from 'lucide-react';

const SidebarItem = ({ label, path, icon, badge, active, children, isOpen, onToggle, isCollapsed, onClick }: any) => {
  const hasChildren = children && children.length > 0;
  const isParentActive = hasChildren && children.some((child: any) => child.active);
    ...
    <div className="flex flex-col gap-1">
      {hasChildren ? (
        ...
        >
          ...
          <div className={`shrink-0 transition-all duration-300 ${isParentActive ? 'scale-110 text-blue-400 drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]' : 'group-hover:text-slate-200'}`}>
            {React.cloneElement(icon as React.ReactElement<{ size?: number }>, { size: 20 })}
          </div>
          
          {!isCollapsed && (
            <>
              <span className={`flex-1 text-sm transition-colors duration-300 ${isParentActive ? 'font-bold text-blue-50' : 'font-medium'}`}>
                {label}
              </span>
              <ChevronDown 
                size={16} 
        ...
        >
          ...
          {!isCollapsed && (
            <>
              <span className={`flex-1 text-sm ${active ? 'font-bold' : 'font-medium'}`}>{label}</span>
              {badge && (
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold transition-transform group-hover:scale-110 ${
...
export default function AdminSidebar({ isCollapsed, isMobileOpen, onToggleCollapse, onCloseMobile }: AdminSidebarProps) {
  ...
  const navigation = [
    {
      ...
      items: [
        ...
        { label: 'Đơn hàng', path: '/admin/orders', icon: <ShoppingCart />, badge: 5 },
      ]
    },
    {
      group: 'KINH DOANH',
      items: [
        ...
        { 
          ...
          icon: <Wallet />,
          children: [
            { label: 'Tổng quan', path: '/admin/finance', active: pathname === '/admin/finance' },
            { label: 'Giao dịch', path: '/admin/finance/transactions', active: pathname === '/admin/finance/transactions' },
            { label: 'Thanh toán Seller', path: '/admin/finance/payments', active: pathname === '/admin/finance/payments' },
          ]
        },
      ]
    },
    ...
    <aside className={sidebarClasses}>
      <div className={`h-20 flex items-center justify-between px-6 border-b border-slate-800/50 shrink-0`}>
        ...
        >
          ...
          <span className={`font-bold text-xl tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent transition-opacity duration-300 ${isCollapsed ? 'lg:opacity-0' : 'opacity-100'}`}>
            STAY-GO
          </span>
        </Link>
      ...
      <div className="flex-1 py-6 flex flex-col gap-8 overflow-y-auto custom-scrollbar overflow-x-hidden">
        {navigation.map((group, gIdx) => (
          <div key={gIdx} className="flex flex-col gap-1">
            <h4 className={`px-7 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-2 transition-opacity duration-300 ${isCollapsed ? 'lg:opacity-0 lg:h-0' : 'opacity-100'}`}>
              {group.group}
            </h4>
            <div className="flex flex-col gap-1">
              {group.items.map((item, idx) => (
                <SidebarItem 
                  ...
                  active={item.path ? (pathname === item.path || (item.path !== '/admin' && pathname.startsWith(item.path))) : false}
                  isOpen={(item as any).id === openSubmenu}
                  onClick={onCloseMobile}
                  onToggle={() => setOpenSubmenu(openSubmenu === (item as any).id ? null : ((item as any).id || null))}
                />
              ))}
            </div>
          </div>
      ...
      <div className="p-4 border-t border-slate-800/50 shrink-0">
        <div className={`px-2 py-3 bg-slate-800/30 rounded-2xl flex items-center gap-3 border border-slate-800/50 relative overflow-hidden transition-all duration-300 ${isCollapsed ? 'lg:justify-center' : ''}`}>
          <div className="relative shrink-0 tooltip-trigger cursor-help" data-tooltip="Admin Manager">
            <img 
          ...
          <div className={`flex-1 min-w-0 transition-all duration-300 ${isCollapsed ? 'lg:opacity-0 lg:w-0' : 'opacity-100 w-auto'}`}>
            <p className="text-sm font-bold text-white truncate">Admin Manager</p>
            <p className="text-[10px] text-slate-500 truncate">Sàn TMĐT STAY-GO</p>
          </div>
