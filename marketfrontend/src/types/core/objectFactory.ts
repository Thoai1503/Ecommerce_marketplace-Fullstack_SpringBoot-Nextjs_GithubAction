...
export class ObjectsFactory<T> {
   ...
   */
  constructor(
    { path, model, headers }: IServiceConstructorData,
    queryKeys?: Record<string, string>,
  ) {
    this.service = new ApiService({ path });
    this.path = path;
  ...
  createFormData(data: unknown, config: AxiosRequestConfig = {}) {
    ...
    if (config.method === "POST")
      return this.service.postFormData({ url, ...rest, data });
