...
export class ApiService {
          ...
          try {
            if (typeof window !== "undefined") {
              ...
              console.log("🔄 Refresh token response parsed:", {
                ...
                accessTokenPreview: newToken
                  ? newToken.substring(0, 20) + "..."
                  : null,
                refreshTokenPreview: newRefreshToken
              ...
              if (newToken) {
                ...
                if (typeof window !== "undefined") {
                  ...
                  if (newRefreshToken) {
                    ...
                    console.warn(
                      "⚠️ No new refreshToken in response, keeping old one",
                    );
                  }

                  // Cập nhật expiresIn
                  const expiresIn =
                    nestedData?.expiresIn || fullData?.expiresIn;
                  if (expiresIn) {
                    const expiresTime = {
                      value: expiresIn,
   ...
   * @param config
   */
  get<T>(config: AxiosRequestConfig = {}): Promise<any> {
  ...
  postFormData<T>(config: AxiosRequestConfig = {}) {
    ...
    return this.http.post<T>(`${process.env.apiEndpoint}${url}`, data, {
      ...params,
      headers: {
        Authorization: token ? `Bearer ${token}` : undefined,
