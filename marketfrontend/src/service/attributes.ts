...
export const createAttribute = async (
  data: Omit<Attribute, "id" | "attributeCode" | "valuesCount" | "createdAt">,
): Promise<Attribute> => {
  await delay(800);
...
export const getAttributeValues = async (
  ...
  await delay(500);
  return MOCK_VALUES.filter((v) => v.attributeId === attributeId).sort(
    (a, b) => a.displayOrder - b.displayOrder,
  );
};

export const createAttributeValue = async (
  data: Omit<AttributeValue, "id" | "createdAt">,
): Promise<AttributeValue> => {
  await delay(600);
