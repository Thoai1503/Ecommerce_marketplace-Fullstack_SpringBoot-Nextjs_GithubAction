...
const mapShopToSeller = (shop: any): Seller => {
  return {
    ...
    logoUrl:
      shop.shop_logo ||
      `https://ui-avatars.com/api/?name=${(shop.shop_name || "Shop").charAt(0)}&background=0078FF&color=fff`,
...
const mapShipmentToSellerOrder = (shipment: any): SellerOrderShipment => {
  ...
  return {
    shipmentId: toText(
      shipment?.shipmentId ?? shipment?.shipment_id ?? shipment?.id,
    ),
    orderId,
    orderNumber: toText(
...
export const getSellers = async (): Promise<Seller[]> => {
  try {
    const response = await axios.get(`${API_URL}/api/admin/shops`);
...
export const getSellerById = async (
  ...
  try {
    const response = await axios.get(`${API_URL}/api/admin/shops/${id}`);
...
export const getSellerOrderShipments = async (
  id: string,
): Promise<SellerOrderShipment[]> => {
  const response = await axios.get(
    `${API_URL}/api/orders/shipments/shop/${id}`,
    {
      params: {
        page: 1,
        size: 100,
...
export const updateSeller = async (
  ...
  try {
    ...
    if ((data as any).idCardFront !== undefined)
      payload.id_card_front = (data as any).idCardFront;
    if ((data as any).idCardBack !== undefined)
      payload.id_card_back = (data as any).idCardBack;
