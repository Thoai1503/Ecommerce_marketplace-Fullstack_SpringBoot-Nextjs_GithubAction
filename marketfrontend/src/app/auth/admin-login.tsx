...
import { Mail, Lock, Eye, EyeOff, ArrowRight, ShieldCheck, Globe, Server, Users, Activity, LayoutDashboard } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const [email, setEmail] = useState('admin@vietcommerce.com');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (email && password.length >= 6) {
        localStorage.setItem('token', 'mock_access_token_' + Date.now());
        toast.success("Xác thực thành công. Đang truy cập hệ thống...");
        
        // Delay nhỏ để người dùng đọc thông báo trước khi chuyển trang
        setTimeout(() => {
            navigate('/admin');
        }, 800);
      } else {
      ...
      <div className="hidden lg:flex w-7/12 bg-[#0f172a] relative overflow-hidden flex-col justify-between p-16 text-white">
            ...
            <div className="flex items-center gap-3 mb-8">
                <div className="w-12 h-12 bg-white/10 backdrop-blur-md rounded-xl flex items-center justify-center border border-white/10 overflow-hidden">
                    <img src="/logo.png" alt="Logo" className="w-full h-full object-cover" onError={(e) => {
                        ...
                        (e.target as HTMLImageElement).parentElement!.innerHTML = '<span class="font-black text-xl text-white italic">V</span>';
                    }} />
                </div>
                <div className="flex flex-col">
                    <span className="text-2xl font-black tracking-tight text-white leading-none">VietCommerce Hub</span>
                    <span className="text-xs font-bold text-indigo-300 tracking-widest uppercase mt-1">Administrator</span>
                </div>
            </div>
        </div>
        ...
        <div className="relative z-10 max-w-xl mb-12">
            <h2 className="text-5xl font-black mb-8 leading-tight tracking-tight text-white">
                ...
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400">hệ thống toàn cầu.</span>
            </h2>
            <div className="space-y-8">
                <div className="flex items-start gap-5 group">
                    ...
                    <div>
                        ...
                        <p className="text-slate-400 text-sm leading-relaxed">Kiểm soát phân quyền, xác thực danh tính và quản lý hồ sơ Seller/Customer tập trung.</p>
                    </div>
                </div>
                <div className="flex items-start gap-5 group">
                    <div className="p-3 bg-white/5 border border-white/10 rounded-2xl shrink-0 group-hover:bg-indigo-600 group-hover:border-indigo-500 transition-colors duration-300">
                    ...
                    <div>
                        ...
                        <p className="text-slate-400 text-sm leading-relaxed">Theo dõi sức khỏe hệ thống, lưu lượng truy cập và các chỉ số hiệu suất chính (KPIs) theo thời gian thực.</p>
                    </div>
                </div>
                <div className="flex items-start gap-5 group">
                    <div className="p-3 bg-white/5 border border-white/10 rounded-2xl shrink-0 group-hover:bg-indigo-600 group-hover:border-indigo-500 transition-colors duration-300">
                    ...
                    <div>
                        ...
                        <p className="text-slate-400 text-sm leading-relaxed">Thiết lập tham số toàn cục, quản lý danh mục ngành hàng và các thiết lập bảo mật cấp cao.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="relative z-10 flex justify-between items-center text-xs font-bold text-slate-500 uppercase tracking-widest border-t border-white/5 pt-8">
            <span>© 2024 VietCommerce Hub Inc.</span>
      ...
      <div className="w-full lg:w-5/12 flex items-center justify-center p-6 lg:p-12 relative">
        <div className="max-w-[420px] w-full bg-white p-8 lg:p-10 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 relative">
            ...
            <div className="mb-8">
                ...
                <p className="text-slate-500 font-medium text-sm">Đăng nhập để truy cập quyền quản trị cấp cao.</p>
            </div>

            {/* Form */}
            <form onSubmit={handleLogin} className="space-y-5">
                <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Tài khoản quản trị</label>
                ...
                <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Mật khẩu bảo mật</label>
                    <div className="relative group">
                        <input 
                ...
                <div className="flex items-center justify-between pt-1">
                    <label className="flex items-center gap-2.5 cursor-pointer group select-none">
                        <div className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${rememberMe ? 'bg-indigo-600 border-indigo-600' : 'border-slate-300 bg-white group-hover:border-indigo-400'}`}>
                            {rememberMe && <ArrowRight size={10} className="text-white rotate-[-45deg] mt-[-1px]" strokeWidth={4} />}
                        </div>
                        <input type="checkbox" className="hidden" checked={rememberMe} onChange={() => setRememberMe(!rememberMe)} />
                        <span className="text-xs font-bold text-slate-600 group-hover:text-indigo-600 transition-colors">Duy trì đăng nhập</span>
                    </label>
            ...
            <div className="mt-10 pt-6 border-t border-slate-100 flex flex-col items-center gap-3 text-center">
                ...
                <p className="text-[10px] text-slate-400 max-w-xs leading-normal">
                    Truy cập trái phép vào khu vực này được giám sát và ghi lại. IP của bạn: 192.168.1.1
                </p>
            </div>
        </div>
      </div>
