...
export default function AttributeDetail() {
  ...
  const handleSaveModalValue = async (data: { value: string; displayOrder: number }) => {
    try {
      if (editingValue) {
  ...
  return (
    <div className="p-6 lg:p-10 animate-in fade-in duration-500 max-w-5xl mx-auto pb-24 space-y-8">
      <AttributeValueModal 
        ...
        initialValues={editingValue ? [editingValue.value] : values.map(v => v.value)}
        onSave={async (newValues) => {
          try {
            for (let i = 0; i < newValues.length && i < values.length; i++) {
              await updateValue({
                valueId: values[i].id,
                data: { value: newValues[i], displayOrder: values[i].displayOrder },
              });
            }
