...
import { 
  Search, Plus, Edit3, Trash2, CheckCircle, XCircle, 
  Layers, Package, LayoutGrid, List, Folder
} from 'lucide-react';
...
export default function CategoryList() {
  ...
  const { categories, isLoading, deleteCategory, updateCategory, isDeleting } = useCategories();
  ...
  const stats = useMemo(() => {
    ...
    const totalProducts = categories.reduce((sum, c) => sum + c.productStock, 0);
  ...
  const openDeleteModal = (e: React.MouseEvent, id: string, hasProducts: boolean) => {
    e.stopPropagation();
    if (hasProducts) {
  ...
  const handleToggleStatus = async (e: React.MouseEvent, category: Category) => {
    e.stopPropagation();
    try {
      const newStatus = category.status === 'ACTIVE' ? 'HIDDEN' : 'ACTIVE';
      await updateCategory({ id: category.id, data: { status: newStatus } });
      toast.success(`Danh mục đã chuyển sang trạng thái: ${newStatus === 'ACTIVE' ? 'Hoạt động' : 'Ẩn'}`);
    } catch (err) {
  ...
  return (
    <div className="p-6 lg:p-8 animate-in fade-in duration-500 space-y-6 pb-24">
      <ConfirmationModal 
        ...
        title="Xóa danh mục?"
        description="Hành động này sẽ xóa danh mục vĩnh viễn và không thể hoàn tác."
      ...
      <div className="flex flex-col gap-6">
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
               ...
               <p className="text-sm text-slate-500 font-medium">Tổ chức và quản lý cấu trúc ngành hàng sản phẩm.</p>
            </div>
            <button 
         ...
         <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
               ...
               <div>
                  ...
                  <p className="text-xs font-bold text-slate-400 uppercase">Tổng danh mục</p>
               </div>
            </div>
            <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
               ...
               <div>
                  <p className="text-2xl font-black text-slate-800">{stats.totalProducts.toLocaleString()}</p>
                  <p className="text-xs font-bold text-slate-400 uppercase">Sản phẩm liên kết</p>
               </div>
            </div>
         </div>
      </div>
        ...
        <div className="flex-1 bg-slate-50/50 p-6">
           {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center">
                 ...
                 <h3 className="text-slate-500 font-bold mb-1">Không tìm thấy danh mục</h3>
                 <p className="text-slate-400 text-sm">Thử điều chỉnh từ khóa hoặc bộ lọc.</p>
              </div>
           ) : (
              <>
                 {viewMode === 'grid' ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-in fade-in duration-300">
                       {paginatedCategories.map(category => (
                          <div 
                             key={category.id}
                             onClick={() => router.push(`/admin/categories/industries/${category.id}`)}
                          ...
                          >
                             ...
                             <div className="aspect-[4/3] rounded-2xl bg-slate-100 overflow-hidden relative mb-4">
                                <img src={category.thumbnailUrl} alt={category.name} className={`w-full h-full object-cover transition-transform duration-500 ${category.status === 'HIDDEN' ? 'grayscale' : 'group-hover:scale-105'}`} />
                                ...
                                <div className="absolute top-3 right-3 z-10" onClick={(e) => e.stopPropagation()}>
                                   ...
                                   >
                                      <div className={`absolute top-1 left-1 w-3 h-3 bg-white rounded-full shadow-sm transition-transform duration-200 ${category.status === 'ACTIVE' ? 'translate-x-5' : 'translate-x-0'}`}></div>
                                   </div>
                                </div>
                             </div>
                             ...
                             <div className="flex-1 flex flex-col">
                                ...
                                <div className="mt-auto pt-4 border-t border-slate-50">
                                   <div className="flex items-center justify-between text-xs mb-1.5">
                                      <span className="font-bold text-slate-400 uppercase">Sản phẩm</span>
                                      <span className="font-black text-slate-800">{category.productStock} sản phẩm</span>
                                   </div>
                                   <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                      <div 
                                         className="h-full bg-blue-500 rounded-full" 
                                         style={{ width: `${Math.min(100, category.productStock > 0 ? (category.productStock / 100) * 10 : 0)}%` }}
                                      ></div>
                                   </div>
                                </div>
                             </div>
                             ...
                             <div className="flex gap-2 mt-4 pt-0 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-200">
                                <button 
                                   onClick={(e) => { e.stopPropagation(); router.push(`/admin/categories/industries/${category.id}/edit`); }}
                                   className="flex-1 py-2 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-xl text-xs font-bold transition-colors"
                                >
                                   Sửa
                                </button>
                                <button 
                                   onClick={(e) => openDeleteModal(e, category.id, category.productStock > 0)}
                       ...
                       <table className="w-full text-left border-collapse min-w-[800px]">
                          <thead>
                             <tr className="bg-slate-50/50 border-b border-slate-100">
                                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Tên danh mục</th>
                                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Mã ID</th>
                             ...
                             {paginatedCategories.map((category) => (
                                ...
                                >
                                   <td className="px-6 py-4">
                                      <div className="flex items-center gap-4">
                                         <div className="w-12 h-12 rounded-xl overflow-hidden border border-slate-200 bg-white shrink-0">
                                            <img src={category.thumbnailUrl} alt={category.name} className={`w-full h-full object-cover ${category.status === 'HIDDEN' ? 'grayscale' : ''}`} />
                                         </div>
                                         <div>
                                            <p className="text-sm font-bold text-slate-800">{category.name}</p>
                                            <p className="text-[10px] text-slate-400 truncate max-w-[200px]">{category.description}</p>
                                         </div>
                                      </div>
                                   </td>
                                   <td className="px-6 py-4">
                                   ...
                                   <td className="px-6 py-4 text-center">
                                      <span className="text-sm font-black text-slate-800">{category.productStock}</span>
                                   </td>
