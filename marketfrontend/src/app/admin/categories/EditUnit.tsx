...
import { ChevronLeft, Save, Scale, Ruler, Box, Droplets, HelpCircle, Eye } from 'lucide-react';
...
const TypeConfig: Record<UnitType, { label: string; icon: any; color: string; bgColor: string }> = {
  WEIGHT: { label: 'Weight', icon: <Scale size={20} />, color: 'text-amber-600', bgColor: 'bg-amber-50' },
  LENGTH: { label: 'Length', icon: <Ruler size={20} />, color: 'text-blue-600', bgColor: 'bg-blue-50' },
  VOLUME: { label: 'Volume', icon: <Droplets size={20} />, color: 'text-cyan-600', bgColor: 'bg-cyan-50' },
  QUANTITY: { label: 'Quantity', icon: <Box size={20} />, color: 'text-purple-600', bgColor: 'bg-purple-50' },
  OTHER: { label: 'Other', icon: <HelpCircle size={20} />, color: 'text-slate-500', bgColor: 'bg-slate-50' },
};
...
export default function EditUnit() {
  ...
  const isEditMode = !!id;
  const { unit, isLoading, createUnit, updateUnit, isSaving } = useUnitDetail(id || '');
    ...
    <div className="p-6 lg:p-10 animate-in fade-in duration-500 max-w-5xl mx-auto pb-24 space-y-6">
      ...
      <div className="flex items-center justify-between">
         <div className="flex items-center gap-4">
            <button onClick={() => router.push('/admin/categories/units')} className="p-2.5 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 text-slate-500 transition-all">
              <ChevronLeft size={20} />
            </button>
            <div>
              <h1 className="text-2xl font-black text-slate-800">{isEditMode ? 'Edit Unit' : 'Add Unit'}</h1>
              <p className="text-sm text-slate-500 font-medium mt-1">Standardize how product quantities are measured.</p>
            </div>
         </div>
         <div className="flex items-center gap-3">
            <button onClick={() => router.push('/admin/categories/units')} className="px-5 py-3 text-sm font-bold text-slate-500 hover:bg-slate-100 rounded-xl border-0 bg-transparent transition-all">
              Cancel
            </button>
            <button 
            ...
            >
              <Save size={18} /> {isSaving ? 'Saving...' : (isEditMode ? 'Edit Unit' : 'Save Unit')}
            </button>
         </div>
      </div>
            ...
            <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm p-8 space-y-8">
               ...
               {isEditMode && (
                  <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-between">
                     <div>
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Internal ID</p>
                        <p className="text-sm font-black text-slate-800 font-mono">{formData.id}</p>
                     </div>
               ...
               <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                     <label className="text-sm font-bold text-slate-700">Unit Type <span className="text-red-500">*</span></label>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     <div className="space-y-2">
                        <label className="text-sm font-bold text-slate-700">Unit Label <span className="text-red-500">*</span></label>
                        <input 
                          type="text" 
                     ...
                     <div className="space-y-2">
                        ...
                        <p className="text-[10px] text-slate-400">This will be displayed next to product attributes.</p>
                     </div>
                  </div>
         ...
         <div className="space-y-6">
            <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm p-6 sticky top-6">
               ...
               <div className="space-y-6">
                  ...
                  <div>
                     <p className="text-xs font-bold text-slate-500 mb-2 text-center">Product Detail View</p>
                     <div className="p-4 bg-white border border-slate-200 rounded-xl shadow-sm">
                        ...
                        <div className="p-3 bg-slate-50 rounded-lg border border-slate-100 flex justify-between items-center">
                           <span className="text-sm font-bold text-slate-600">{formData.label || 'Attribute'}</span>
                  ...
                  <div>
                     <p className="text-xs font-bold text-slate-500 mb-2 text-center">Attribute Selection</p>
                     <div className="p-4 bg-white border border-slate-200 rounded-xl shadow-sm">
                        <div className="flex gap-2 flex-wrap">
                           {['5', '10', '25'].map((val, i) => (
                              <div key={val} className={`px-3 py-2 rounded-lg border text-sm font-bold ${i === 1 ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 text-slate-600'}`}>
                                 {val} {formData.symbol}
                              </div>
                           ))}
                        </div>
