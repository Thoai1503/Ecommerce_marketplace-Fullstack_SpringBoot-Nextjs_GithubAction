...
export default function EditSeller() {
  ...
  const validateForm = async () => {
    ...
    const payloadToValidate = {
      ...formData,
      password:
        !isEditMode && authMethod === "manual" ? manualPassword : undefined,
    };
                  ...
                  <div className="space-y-3">
                    ...
                    {errors.password ? (
                      ...
                      <div className="flex gap-2">
                        <span
                          className={`h-1 flex-1 rounded-full ${manualPassword.length > 0 ? (manualPassword.length < 8 ? "bg-red-200" : "bg-green-400") : "bg-slate-100"}`}
                        ></span>
                        <span
                          className={`h-1 flex-1 rounded-full ${manualPassword.length >= 8 ? "bg-green-400" : "bg-slate-100"}`}
                        ></span>
                        <span
                          className={`h-1 flex-1 rounded-full ${manualPassword.length >= 12 ? "bg-green-400" : "bg-slate-100"}`}
                        ></span>
                      </div>
                    )}
