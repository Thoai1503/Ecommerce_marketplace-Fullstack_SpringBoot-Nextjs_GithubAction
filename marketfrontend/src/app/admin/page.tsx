...
const StatCard = ({
  ...
  <div className="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex flex-col justify-between h-full hover:shadow-md transition-all duration-300">
    <div className="flex justify-between items-start mb-2">
      ...
      <div
        className={`flex items-center gap-1 text-[11px] font-bold px-2 py-1 rounded-lg shrink-0 ${isPositive ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"}`}
      >
        {isPositive ? "+" : ""}
        {change}%
    ...
    <div className="min-w-0">
      <h3
        className="text-xl lg:text-2xl font-black text-slate-800 tracking-tight truncate"
        title={String(value)}
      >
        {value}
      </h3>
      <p
        className="text-xs font-bold text-slate-400 uppercase tracking-wide mt-1 truncate"
        title={title}
      >
        {title}
      </p>
...
const StatusBadge = ({ status }: { status: OrderStatus }) => {
  ...
  const style =
    config[status] || "bg-slate-100 text-slate-600 border-slate-200";

  return (
    <span
      className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider border ${style}`}
    >
      {status}
    </span>
  );
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white/95 backdrop-blur-md border border-slate-100 p-4 rounded-2xl shadow-xl animate-in zoom-in-95 duration-200">
        ...
        <div
          className={`pt-2 border-t border-slate-100 flex items-center gap-1 text-xs font-bold ${isGrowthPositive ? "text-green-600" : "text-red-600"}`}
        >
          {isGrowthPositive ? (
          ...
          <span>
            {Math.abs(growth).toFixed(1)}% compared to the previous period
          </span>
        </div>
      </div>
    );
...
const PaymentMethodIcon = ({ method }: { method: string }) => {
  ...
  if (m.includes("credit") || m.includes("card") || m.includes("visa"))
  ...
  if (m.includes("wallet") || m.includes("momo") || m.includes("zalo"))
    return <Wallet size={14} className="text-blue-600" />;
  if (m.includes("transfer") || m.includes("bank"))
    return <ArrowUpRight size={14} className="text-orange-600" />;
...
const TimeAgo = ({ dateStr }: { dateStr: string }) => {
  ...
  useEffect(() => {
    const updateTime = () => {
      ...
      if (mins < 1) setTimeAgo("Just finished");
      else if (mins < 60) setTimeAgo(`${mins} minutes ago`);
...
export default function AdminDashboard() {
  ...
  const chartTotalPrevRevenue = chartData.reduce(
    (sum: number, item: any) => sum + (item.prevRevenue || 0),
    0,
  );
  const chartTrend =
    chartTotalPrevRevenue > 0
      ? ((chartTotalRevenue - chartTotalPrevRevenue) / chartTotalPrevRevenue) *
        100
      : 0;
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${chartTrend >= 0 ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"}`}
                >
                  {chartTrend >= 0 ? "+" : ""}
                  {chartTrend.toFixed(1)}%
                </span>
              </div>
              <div className="flex items-baseline gap-2 mt-1">
                <span className="text-2xl font-black text-slate-800">
                  {chartTotalRevenue.toLocaleString()}₫
                </span>
        ...
        <div className="lg:col-span-4 bg-white p-6 rounded-[24px] border border-slate-200 shadow-sm flex flex-col h-full">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              ...
              className="text-xs font-bold text-slate-400 hover:text-slate-600 bg-slate-50 px-3 py-1.5 rounded-lg transition-colors"
            >
              View All
            </button>
          </div>
          ...
          <div className="space-y-4 flex-1">
            {topProducts.map((product, idx) => (
              <div
                key={product.id}
                className="flex items-center gap-4 group cursor-pointer hover:bg-slate-50 p-2 rounded-2xl transition-all border border-transparent hover:border-slate-100 hover:shadow-sm"
              >
                {/* Rank & Image */}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-[10px] font-bold uppercase text-slate-500 bg-slate-100 px-1.5 rounded truncate max-w-[80px]">
                      {product.category}
                    </span>
                    {product.stock < 10 && (
                ...
                <div className="text-right shrink-0 flex flex-col items-end">
                  ...
                  <div className="flex items-center gap-1 text-[10px]">
                    ...
                    {product.growth !== 0 && (
                      <span
                        className={`font-bold ${product.growth > 0 ? "text-green-500" : "text-red-500"}`}
                      >
                        {product.growth > 0 ? "↑" : "↓"}{" "}
          ...
          <button
            ...
            onClick={() => router.push("/admin/products")}
          >
            <TrendingUp size={14} /> View Detailed Report
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium">
              Track newly generated orders on the system.
            </p>
          </div>
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100 text-left">
                ...
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right pr-8">
                  Actions
                </th>
              </tr>
            </thead>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 pl-8">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                          ...
                          {isPending && (
                            <span className="text-[9px] font-bold bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded border border-amber-200 animate-pulse">
                              NEW
                            </span>
                          )}
                        </div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-black shrink-0 border-2 border-white shadow-sm ${["A", "C", "E", "G"].includes(order.customerName.charAt(0)) ? "bg-blue-100 text-blue-600" : "bg-purple-100 text-purple-600"}`}
                        >
        ...
        <div className="p-4 border-t border-slate-100 text-center bg-slate-50/50">
          <button
            ...
            className="text-xs font-bold text-blue-600 hover:text-blue-700 hover:underline flex items-center justify-center gap-1 mx-auto"
          >
