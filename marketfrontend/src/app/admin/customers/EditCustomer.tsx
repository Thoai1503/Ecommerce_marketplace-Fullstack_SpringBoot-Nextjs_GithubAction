    ...
    <div className="p-6 lg:p-10 animate-in fade-in duration-500 max-w-5xl mx-auto pb-24 space-y-6">
      ...
      <div className="flex items-center justify-between">
         <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-slate-800">Chỉnh sửa thông tin</h1>
              <p className="text-sm text-slate-500 font-medium mt-1">Cập nhật hồ sơ khách hàng {customer.accountCode}</p>
            </div>
         </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
           ...
           <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm p-8 space-y-6">
              <div className="flex items-center justify-between">
                 ...
                 <span className="text-xs font-bold text-slate-400">Chỉnh sửa tại trang chi tiết</span>
              </div>
              <div className="space-y-3">
                 {customer.addresses.map((addr, idx) => (
                    <div key={idx} className="p-3 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-between">
                       ...
                       {addr.isDefault && <span className="text-[10px] font-bold bg-blue-100 text-blue-600 px-2 py-1 rounded">Mặc định</span>}
                    </div>
                 ))}
              </div>
           </div>
        ...
        <div className="space-y-8">
           ...
           <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm p-6 space-y-6">
              <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest">Trạng thái</h3>
              ...
              <div className="space-y-4">
                <div className="relative">
                   ...
                   >
                      ...
                      <option value="BANNED">❌ Đã chặn (Banned)</option>
                   </select>
              ...
              <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl">
                 <div className="flex justify-between items-center mb-1">
                   <span className="text-xs font-bold text-slate-500 uppercase">Ngày tham gia</span>
                   <span className="text-xs font-bold text-slate-700">{new Date(customer.joinedAt).toLocaleDateString()}</span>
                 </div>
                 <div className="flex justify-between items-center">
                   <span className="text-xs font-bold text-slate-500 uppercase">Lần cuối mua hàng</span>
           ...
           <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm p-6 space-y-6">
              <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest">Ảnh đại diện</h3>
              <div className="flex flex-col items-center">
                 ...
                 <button className="text-xs font-bold text-blue-600 hover:underline">Thay đổi ảnh</button>
              </div>
           </div>
